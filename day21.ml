...
let silver monkeys =
  ...
  let rec loop monkeys =
    ...
    | {id;message=Number(value)} :: ms ->
       ...
       loop ms
    | {id;message=Op(id1,id2,f,_,_)} :: ms when mem monkeys_hash id1 && mem monkeys_hash id2 ->
...
let rec solve_root_deps_opt monkeys search_id =
  ...
  | {id=_;message=Op(i1,i2,_,_,_)} when i2 = "humn" || i1 = "humn" -> None
...
let gold monkeys =
  ...
  let left_sum, right_sum = solve_root_deps_opt monkeys left, solve_root_deps_opt monkeys right in
  Printf.printf "%s=%d - %s=%d\n" left (Option.default 0 left_sum) right (Option.default 0 right_sum)
